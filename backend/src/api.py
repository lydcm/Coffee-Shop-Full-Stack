...
def get_drinks():
    ...
    all_drinks_short_format = [drink.short() for drink in all_drinks]

    return jsonify({"success": True, "drinks": all_drinks_short_format})
...
def get_drinks_details(payload):
    ...
    all_drinks_long_format = [drink.long() for drink in all_drinks]

    return jsonify({"success": True, "drinks": all_drinks_long_format})
...
def update_drink(payload, drink_id):
    ...
    if updated_recipe:
        drink_to_edit.recipe = f"""{body['recipe']}"""

    drink_to_edit.update()

    return jsonify({"success": True, "drinks": [Drink.long(drink_to_edit)]})
...
def delete_drink(payload, drink_id):
    ...
    if not drink_to_delete:
        abort(404, {
            "message": f"Drink id {drink_id} does not exist in the database."
        })
